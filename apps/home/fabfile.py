"""
KittyHome deployment - fab setup / fab deploy / fab smoke / fab logs / fab status / fab rollback
"""
import os
import subprocess
import sys
...
def setup(ctx):
    ...
    _put_rendered(c, LOCAL_ROOT / "deploy" / "kittyhome.service", "/tmp/kittyhome.service")
    _put_rendered(c, LOCAL_ROOT / "deploy" / "kittyhome.nginx", "/tmp/kittyhome.nginx")

    c.sudo("cp /tmp/kittyhome.service /etc/systemd/system/kittyhome.service")
    c.sudo("cp /tmp/kittyhome.nginx /etc/nginx/sites-enabled/kittyhome")
    c.sudo("systemctl daemon-reload")
    c.sudo("systemctl enable kittyhome")
    c.sudo("nginx -t")
