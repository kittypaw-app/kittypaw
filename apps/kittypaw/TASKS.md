...
현재 진행 중인 Projects 작업은 없음.

---

## 📌 Parked / Historical Working Notes

---

### Projects Phase 2: PTY Job Runtime / Interactive Driver Sessions ✅ 완료
> Spec: `docs/superpowers/specs/2026-05-10-project-pty-job-runtime-design.md`
> Plan: `docs/superpowers/plans/2026-05-10-project-pty-job-runtime-implementation.md`
...
- [x] **T0: 구현 계획 작성** — `docs/superpowers/plans/2026-05-10-project-pty-job-runtime-implementation.md` 에 runtime PTY adapter, `/api/v1/jobs/{job}/input`, web input control, Projects.appendJobInput tool, deterministic/live smoke 테스트 범위를 작업 단위로 정리.
- [x] **T1: store/runtime/API/tool/UI 구현** — PTY Job mode, input forwarding, transcript events, web controls (`784e7e4`, `757efc2`, `e6300d7`, `255bac0`, `16fe615`, `0c158e8`).
- [x] **T2: 검증 및 리뷰** — deterministic tests + opt-in live shell PTY smoke (`c4d1dab`), `go test ./store ./engine ./server -count=1`, `KITTYPAW_LIVE_PTY=1 go test ./engine -run TestProjectJobRuntimeLiveShellPTYEcho -count=1`, `make test-ci`.
...
- [ ] **Kakao rich response E2E** — runner가 이미지 포함 응답을 만들었을 때 Kittypaw → Kakao relay → callback body가 `simpleImage` 등 Kakao rich response shape로 나가는지 검증.
...
- [ ] **Account-scoped event stream / SSE** — OpenClaw식 global broadcast는 Kittypaw multi-account 경계와 맞지 않음. 필요 시 `/api/v1/events` 또는 SSE를 account auth + permission check 기반으로 별도 설계.

---

## ✅ Archive

완료 순 (최신 → 과거). 확정된 핵심 커밋 해시는 함께 첨부.

### 2026-05-11

- **Conversation Thread Persistence Foundation** — `conversations` first-class
  thread index, `conversation_id` on turns/checkpoints/compactions, scoped
  history/forget/compact/checkpoint rollback, conversation list/info/messages
  APIs, CLI `--conversation-id`, and channel-derived general threads
  (`general:<channel>:<chat-or-session>`) added. ToolTrace remains structured
  JSON on assistant turns; separate indexing/redaction policy is deferred.

### 2026-05-10

- **Projects Job Runtime Phase 1.5** — approved Job `one_shot` 실행을 account-managed git worktree 기반 runtime으로 연결. store lifecycle, API/tool/web controls, shell live smoke, insertion-ordered events, bounded error capture, concurrent start cleanup, running cancel propagation, default account reload wiring hardening 포함. 릴리스 `kittypaw/v0.5.21`; 핵심 커밋 `b68e06f`, `f0c89eb`, `c2cdb32`, `db5f0a5`, `1b38766`, `81ff925`, `5742c2b`, `5880c2d`, `9392a45`.

- **Projects Board Replacement MVP1** — Workspace/Kanban 사용자 표면을 Project Folder 중심의 Projects/Board 흐름으로 대체. DB 기반 Project/Ticket/Job/Driver/Staff Assignment/Conversation scope, `/api/v1/projects|tickets|jobs|drivers`, Projects web board, project/ticket scoped chat, Project Brief Draft commit, kickoff scan, File tool project scoping, deterministic + live LLM e2e 회귀를 포함. 릴리스 `kittypaw/v0.5.20`; 핵심 커밋 `e518a59`, `9dd6b03`, `27cde64`, `e0d492f`, `6ceaafd`, `7bda4c0`, `8c84f9b`, `e885523`, `b6ed65a`.
