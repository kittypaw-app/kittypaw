...
> Spec: `docs/superpowers/specs/2026-05-09-projects-board-design.md`
> Plan: `docs/superpowers/plans/2026-05-09-projects-board-implementation.md` (작성 예정)
> Worktree: `main` worktree에서 진행 (사용자 요청)

Workspace/Kanban 사용자 표면을 Project Folder 중심의 Projects/Board 흐름으로 대체한다. 저장은 계정 DB 기준이며, Project/Ticket/Job/Driver/Staff Assignment/Conversation scope가 새 핵심 모델이다.

- [x] **D1**: 개념 설계와 MVP1 범위 확정 (`77dff0c`).
- [ ] **T0**: 구현 계획 작성, 기존 Workspace/Kanban 영향 범위 확인, 마이그레이션 전략 확정.
- [ ] **T1**: DB/store 모델 추가 — projects, tickets, ticket actions/messages, jobs, drivers, staff assignments, conversation scope.
- [ ] **T2**: API 추가 — `/api/v1/projects`, `/api/v1/tickets`, `/api/v1/jobs`, `/api/v1/drivers`.
- [ ] **T3**: Project Folder 생성/확인, kickoff, structure scan, brief draft/commit 흐름.
- [ ] **T4**: Projects/Board UI 구현 및 기존 Workspace/Kanban 사용자 표면 제거.
- [ ] **T5**: Engine `Projects` tool, slash command, project/ticket chat scope 연결.
- [ ] **T6**: Job planning records와 Driver 설정 UI/API 추가. 실제 Codex/Claude 실행은 후속 phase.
- [ ] **T7**: 회귀, E2E, LLM 호출 포함 전수테스트 경로 정리.
