...
현재 앱 작업은 새 phase 설계 전 대기 상태다. Projects MVP1 은 Archive 로 이동했다.
...
- [ ] **D1: 설계 확정** — MVP1 의 planned/approved `Job` record를 실제 실행으로 연결한다. Codex/Claude/shell `Driver`, `one_shot`/PTY/tmux 모드, worktree/branch 정책, bounded log 저장, 사용자 개입/중단 UX, approval/autonomy 경계를 먼저 spec 으로 확정한다.
