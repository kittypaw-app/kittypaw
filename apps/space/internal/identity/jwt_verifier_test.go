...
func TestJWTCredentialVerifierVerifiesAPIClientToken(t *testing.T) {
	...
	token := signTestJWT(t, map[string]any{
		"sub":   "user_1",
		"iss":   IssuerKittyAPI,
		"aud":   []string{AudienceKittyAPI, AudienceKittySpace},
	...
	assertStringSlice(t, got.Audiences, []string{AudienceKittyAPI, AudienceKittySpace})
...
func TestJWTCredentialVerifierRejectsLegacyIssuerAudience(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			iss:  "kittyapi",
			aud:  []string{"kittyapi", "kittyspace"},
		},
		{
			name: "legacy issuer",
			iss:  "kittyapi",
		...
		},
		{
			...
			iss:  IssuerKittyAPI,
			aud:  []string{AudienceKittyAPI, "kittyspace"},
		},
	}
...
func TestJWTCredentialVerifierRejectsInvalidAPIClientClaims(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "missing issuer",
			claims: map[string]any{
				"sub": "user_1", "aud": []string{AudienceKittyAPI, AudienceKittySpace},
				"scope": []string{string(ScopeChatRelay), string(ScopeModelsRead)}, "v": CredentialVersion1,
			},
		},
		{
			name: "missing kittyspace audience",
			claims: map[string]any{
		...
		},
		{
			...
			claims: map[string]any{
				"sub": "user_1", "iss": IssuerKittyAPI, "aud": []string{AudienceKittyAPI, AudienceKittySpace},
